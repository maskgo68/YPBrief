...
它会监听你配置的 YouTube 频道和播放列表，获取视频元数据和字幕，生成单视频总结，再把新更新合成为可读的每日或定时简报，并通过 Telegram、飞书或 Email 推送。Web UI 更像一个实用的操作台，用于来源管理、提示词调整、手动运行、自动任务、阅读、导出和排错。
...
- 通过 Telegram、飞书和 Email 推送简报。
- 自动任务通知覆盖三类结果：成功日报、无更新提醒、失败简短告警；失败告警只包含任务名、来源/频道、视频标题、发布时间、链接和压缩后的原因，详细后端错误留在运行记录。
...
- 推送：Telegram Bot API、飞书群机器人 Webhook 和 SMTP Email
...
- `delivery.py`：发送 Telegram、飞书和 Email，拆分长消息，生成无更新/失败通知，脱敏敏感值，并记录推送日志。
...
- `llm.py`：包含 OpenAI-compatible、Gemini、Claude、xAI、DeepSeek、SiliconFlow、OpenRouter 和自定义 provider adapter。
...
YPBrief 支持 OpenAI、Gemini、Claude、SiliconFlow、OpenRouter、xAI、DeepSeek 和自定义 OpenAI-compatible provider。
...
Telegram Bot Token 通过 BotFather 创建 bot 获取；`TELEGRAM_CHAT_ID` 是 bot 能访问的个人 chat、群组、频道 username 或频道/群组 id。用于主动推送时，用户需要先给 bot 发过 `/start`，群组/频道场景需要把 bot 加入目标位置并给予必要权限。
...
飞书当前采用群聊“自定义机器人”Webhook，不使用飞书开放平台 App ID / App Secret。创建路径是：飞书群聊 -> 群设置 -> 群机器人 -> 添加机器人 -> 自定义机器人；创建后复制 Webhook URL，若开启签名校验则同时复制 Secret。
...
- 成功生成日报：发送该任务生成的 digest Markdown，标题使用任务名和日期。
- 无更新：当没有新视频、没有失败项，并且任务开启“无更新也推送”时，发送简短无更新提醒。
- 失败告警：当任务没有生成日报且存在失败视频或运行失败时，发送简短告警。正文只保留任务名、频道、来源、视频标题、发布时间、链接和一句压缩后的失败原因；完整 `yt-dlp`、语言回退、HTTP 或 traceback 细节仍保留在 `DailyRuns`、`DailyRunVideos`、`failed.md` 和日志中。
...
本地开发模式保持前端 `45173`、后端 `48787` 双端口，便于 Vite 热更新和后端 API 调试；Docker 镜像中前端静态文件由 FastAPI 托管，对外只需要 `48787` 这一个入口。
...
- 每次运行后可推送 Telegram、飞书或 Email
- 失败时可发送简短失败通知，而不是把详细后端错误展开到消息里
...
- 失败通知会优先于“无更新”通知，避免字幕抓取失败被误解成当天无内容
...
- 本地 `sources.yaml` 属于本地 / Docker / VPS 运行使用，不应用作远端 GitHub Actions 测试的临时数据。
- 测试 GitHub Actions 时，应在 private GitHub 测试仓库中单独创建或更新一个测试用 `sources.yaml`，通常从 `sources.example.yaml` 复制后只保留一两个来源。
- 准备 Actions 测试时，不要覆盖、删除或强制同步操作者本地的 `sources.yaml`、`key.env`、`data/`、`exports/` 或 `logs/`。
- 仓库准备公开前，应清理 private 测试用 `sources.yaml` 和 `actions-exports/` 历史，只发布 example 和产品源码。
...
使用与你选择的 `LLM_PROVIDER` 对应的 API key。例如 `LLM_PROVIDER=xai` 应搭配 `XAI_API_KEY`；`LLM_PROVIDER=gemini` 应搭配 `GEMINI_API_KEY`。
...
如果让助手或自动化工具代为验证 GitHub Actions，应使用远端专用测试来源文件：

- 在 private GitHub 测试仓库中创建一个小型 `sources.yaml`
- 只放一两个近期活跃的频道或播放列表
- 使用 `window=last_3` 或 `window=all_time`，并把 `max_videos_per_source` 设小
- 本地运行配置文件保持不动
...
`--env-file key.env` 只是本地测试便利入口。在线 GitHub Actions 中，凭证仍来自 Secrets，workflow 会在运行时生成临时 `key.env`。`--dry-run` 会跳过 Telegram/飞书/Email 推送和 git commit，但仍会调用 YouTube 和配置的 LLM。
...
- 如果收到任务失败通知，先看通知中的频道、来源、视频标题和链接；通知中的原因是压缩结论，完整错误在 Actions 日志、`failed.md` 或 Web UI 运行记录中。
...
- Settings：YouTube API、代理、LLM providers/models、Telegram、飞书、Email 和访问密码轮换。
